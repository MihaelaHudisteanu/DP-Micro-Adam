...
class DPMicroAdamStP(DPOptimizerStP):
    def __init__(self,
                 params, lr, weight_decay, l2_norm_clip, sigma, batch_size,
                 *,
                 betas=(0.9, 0.999), eps=1e-8, eps2=1e-2, num_grads=10, density=0.01, device='cuda'):
        ...
        for group in self.param_groups:
            group['s_t'] = [torch.ones_like(param.data, device=device) if param.requires_grad else None for param in group['params']]    
    ...
    def optimizer_update_step(self, closure=None):
        ...
        for group, state, p, ind in self.loop_params():
            ...
            group['s_t'][ind] = 1.0 / (vt.sqrt() + eps2)
