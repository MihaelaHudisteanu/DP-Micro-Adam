...
class DPOptimizerStP(torch.optim.Optimizer):
    ...
    def add_noise_then_divide(self):
        ...
        for group, state, p, ind in self.loop_params():
            s_t = group['s_t'][ind]
            group['accum_grads'][ind].add_( # add noise
                other=torch.randn(size=p.shape, requires_grad=False, device=p.device, dtype=p.dtype),
                alpha=std
            )
            group['accum_grads'][ind].div_(self.grad_count)
            group['accum_grads'][ind].div_(s_t)
