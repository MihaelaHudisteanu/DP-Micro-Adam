...
def norm(num_channels, groups=16):
    return nn.GroupNorm(num_groups=groups, num_channels=num_channels, affine=True)   ###might be better with False
...
class Wide_ResNet(nn.Module):
    def __init__(self, depth, widen_factor, num_classes, groups=16):
        super().__init__()
        self.in_planes = 16
    ...
    def _wide_layer(self, block, planes, num_blocks, stride, groups=16):
...
def wide_resnet_16_5(**kwargs):
    model = Wide_ResNet(16, 5, **kwargs)
    return model

def wide_resnet_16_6(**kwargs):
    model = Wide_ResNet(16, 6, **kwargs)
    return model

def wide_resnet_16_8(**kwargs):
    model = Wide_ResNet(16, 8, **kwargs)
    return model

def wide_resnet_40_4(**kwargs):
